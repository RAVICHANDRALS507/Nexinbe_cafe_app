      ...
      <div className="grid grid-cols-5 gap-6">
        {menuItems.map((item) => (
          <div key={item.id} className="p-4 border rounded shadow-lg flex flex-col items-center bg-white">
            ...
            <button onClick={() => handleAddToCart(item)} className="bg-green-500 text-white px-4 py-2 rounded mt-3">
              Add to Cart
            </button>
            ...
            {message[item.id] && <p className="text-green-700 mt-2 text-sm">{message[item.id]}</p>}
          </div>
        ))}
      </div>
    </div>
