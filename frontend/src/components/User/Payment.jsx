...
const Payment = () => { 
  ...
  const handlePayment = async () => { 
    try { 
      const { data } = await axios.post('http://localhost:5000/api/razorpay/checkout', { 
        amount: amount 
      }); 
...
axios.post('http://localhost:5000/api/razorpay/paymentVerification', { 
            ...
            navigate('/cart'); // Return to cart if verification fails 
          } 
        }, 
        modal: { 
          confirm_close: true, 
...
colors" 
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-4 mt-16"> 
        <div className="w-full max-w-md bg-white rounded-2xl shadow-sm p-6 space-y
6"> 
          <h1 className="text-2xl font-bold text-center text-gray-800">Payment 
Details</h1> 
