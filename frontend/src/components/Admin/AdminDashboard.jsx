...
import React, { useEffect, useState } from "react";
import axios from "axios";
import { FaUsers, FaUserShield, FaBoxes } from "react-icons/fa";
import { ToastContainer, toast } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';
...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const response = await axios.get(`${BACKEND_URL}/api/admin/dashboard`); // Corrected URL
        setStats({ users: response.data.users || 0, admins: response.data.admins || 0 });
      } catch {
  ...
  const handleAddAdmin = () => setShowForm(true);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post(`${BACKEND_URL}/api/admin/add`, { name: username, password }); // Corrected URL
      toast.success("Admin added successfully!");
      setUsername("");
      setPassword("");
      setShowForm(false);
  ...
  const handleShowAdmins = async () => {
    try {
      const response = await axios.get(`${BACKEND_URL}/api/admin/admins`); // Corrected URL
      setAdminList(response.data);
      setShowAdmins(true);
      setShowStock(false);
    } catch {
  ...
  const handleViewStock = async () => {
    try {
      const response = await axios.get(`${BACKEND_URL}/api/menu`); // Corrected URL
      setStockList(response.data);
      setShowStock(true);
      setShowAdmins(false);
    } catch {
    ...
    <div className="min-h-screen bg-gray-100 px-4 py-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          {/* Admins Card */}
          <div className="bg-white p-5 rounded-xl shadow">
            <FaUserShield className="text-3xl text-orange-500 mb-2" />
            <h2 className="text-xl font-semibold">Admins</h2>
            <p>{loading ? "..." : stats.admins}</p>
            <div className="mt-4 flex flex-wrap gap-2">
              <button onClick={handleAddAdmin} className="bg-orange-500 text-white px-4 py-2 rounded hover:bg-orange-600">Add Admin</button>
              <button onClick={handleShowAdmins} className="bg-gray-200 px-4 py-2 rounded hover:bg-gray-300">View All</button>
            </div>
          </div>

          {/* Inventory Card */}
          <div className="bg-white p-5 rounded-xl shadow">
            <FaBoxes className="text-3xl text-orange-500 mb-2" />
            <h2 className="text-xl font-semibold">Inventory</h2>
            <p>Manage stock and menu items</p>
            <button onClick={handleViewStock} className="mt-4 bg-orange-500 text-white px-4 py-2 rounded hover:bg-orange-600">View Stock</button>
          </div>
        </div>
        ...
        {showAdmins && (
          <div className="bg-white p-6 rounded-xl shadow mb-8">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold text-orange-500">Admin List</h2>
              <button onClick={() => setShowAdmins(false)} className="bg-red-500 text-white px-3 py-1 rounded hover:bg-red-600">Close</button>
            </div>
        ...
        {showStock && (
          <div className="bg-white p-6 rounded-xl shadow">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold text-orange-500">Stock Items</h2>
              <button onClick={() => setShowStock(false)} className="bg-red-500 text-white px-3 py-1 rounded hover:bg-red-600">Close</button>
            </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 px-4">
          <div className="bg-white p-6 rounded-xl shadow-xl w-full max-w-sm">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <input value={username} onChange={(e) => setUsername(e.target.value)} placeholder="Username" className="w-full px-4 py-2 border rounded" />
